#!/usr/bin/env bash
STORAGE="$(find /db-backup -name "storage_*$DEVNAME*" | head -n 1)"
...
find /db-backup -type f -name "backup_*.sql" -mmin +259200 -exec rm -f {} \;
...
find /db-backup -type f -name "db-data_*" -mmin +64800 -exec rm -f {} \;
