...
./Scripts/shell_prompt.sh "./Scripts/config_etc_const.sh" "${cyan}Step 1. Overwrite constantes.properties\n${nc}" $1
#; get hash password argv are -p password -s salt -f filename (resp in order)
./Scripts/shell_prompt.sh "./Scripts/config_etc_pass.sh ${4} ${5} ${6} ${7} ${8} ${9}" "${cyan}Step 2. Get a hashed password with encryption, PHP encrypts.\n${nc}" $2
...
./Scripts/shell_prompt.sh "migrate-database.sh -y -y -y" "${cyan}Step 3. Migrate database\n${nc}" $3
