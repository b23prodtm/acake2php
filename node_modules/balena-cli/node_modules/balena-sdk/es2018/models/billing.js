...
const getBillingModel = function (deps, opts) {
    ...
    const getOrgId = async (organization) => {
    ...
    const exports = {
         ...
         * });
         */
        getBillingInfo: async (organization) => {
            const orgId = await getOrgId(organization);
            const { body } = await request.send({
                method: 'GET',
         ...
         * });
         */
        updateBillingInfo: async (organization, billingInfo) => {
            const orgId = await getOrgId(organization);
            const { body } = await request.send({
                method: 'PATCH',
                url: `/billing/v1/account/${orgId}/info`,
         ...
         * });
         */
        getInvoices: async (organization) => {
            const orgId = await getOrgId(organization);
            const { body } = await request.send({
                method: 'GET',
         ...
         * });
         */
        async downloadInvoice(organization, invoiceNumber) {
            const orgId = await getOrgId(organization);
