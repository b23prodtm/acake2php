...
const getApplicationModel = function (deps, opts) {
    ...
    const exports = {
        ...
        async create({ name, applicationType, deviceType, parent, organization, }) {
            ...
            const [deviceTypeId, applicationTypeId, parentApplication, organizationId,] = await Promise.all([
                deviceTypeIdPromise,
         ...
         * });
         */
        rename: async (nameOrSlugOrId, newAppName) => {
            try {
                const applicationId = await getId(nameOrSlugOrId);
                await pine.patch({
                    resource: 'application',
        ...
        willTrackNewReleases: async (nameOrSlugOrId) => {
            const { should_track_latest_release } = await exports.get(nameOrSlugOrId, { $select: 'should_track_latest_release' });
        ...
        isTrackingLatestRelease: async (nameOrSlugOrId) => {
            ...
            const application = (await exports.get(nameOrSlugOrId, appOptions));
