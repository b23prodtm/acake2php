...
const getOsModel = function (deps, opts) {
    ...
    const getValidatedDeviceType = async (deviceTypeSlug) => {
     ...
     * @memberof balena.models.os
     */
    const _getDownloadSize = authDependentMemoizer(async (deviceType, version) => {
     ...
     * @memberof balena.models.os
     */
    const _getOsVersions = authDependentMemoizer(async (deviceType) => {
        const { body: { versions, latest }, } = (await request.send({
            method: 'GET',
     ...
     * });
     */
    const getDownloadSize = async function (deviceType, version = 'latest') {
     ...
     * });
     */
    const getSupportedVersions = async function (deviceType) {
        await getValidatedDeviceType(deviceType);
     ...
     * });
     */
    const getMaxSatisfyingVersion = async function (deviceType, versionOrRange = 'latest') {
        await getValidatedDeviceType(deviceType);
     ...
     * });
     */
    const getLastModified = async function (deviceType, version = 'latest') {
        try {
            await getValidatedDeviceType(deviceType);
     ...
     * });
     */
    const download = util_1.onlyIf(!isBrowser)(async function (deviceType, version = 'latest') {
        try {
            await getValidatedDeviceType(deviceType);
            let ver;
            if (version === 'latest') {
     ...
     * });
     */
    const getConfig = async function (nameOrSlugOrId, options) {
        var _a;
     ...
     * });
     */
    const isSupportedOsUpdate = async (deviceType, currentVersion, targetVersion) => {
        await getValidatedDeviceType(deviceType);
     ...
     * });
     */
    const getSupportedOsUpdateVersions = async (deviceType, currentVersion) => {
