...
const getReleaseModel = function (deps, opts) {
    ...
    const tagsModel = buildDependentResource({ pine }, {
        ...
        getResourceId: async (commitOrId) => (await get(commitOrId, { $select: 'id' })).id,
    });
    /**
     ...
     * });
     */
    async function get(commitOrId, options = {}) {
        if (commitOrId == null) {
     ...
     * });
     */
    async function getWithImageDetails(commitOrId, options = {}) {
        ...
        const rawRelease = (await get(commitOrId, util_1.mergePineOptions(baseReleaseOptions, options.release)));
     ...
     * });
     */
    async function getAllByApplication(nameOrSlugOrId, options = {}) {
     ...
     * });
     */
    async function getLatestByApplication(nameOrSlugOrId, options = {}) {
        const [release] = await getAllByApplication(nameOrSlugOrId, util_1.mergePineOptions({
            $top: 1,
            $filter: {
                status: 'success',
            },
     ...
     * });
     */
    async function createFromUrl(nameOrSlugOrId, urlDeployOptions) {
        const appOptions = {
            $select: 'app_name',
            $expand: {
                organization: {
        ...
        const { app_name, organization } = (await applicationModel().get(nameOrSlugOrId, appOptions));
        return await builderHelper().buildFromUrl(organization[0].handle, app_name, urlDeployOptions);
    }
    /**
    ...
    const tags = addCallbackSupportToModule({
         ...
         * });
         */
        async getAllByRelease(commitOrId, options = {}) {
            const releaseOpts = {
                $select: 'id',
                $expand: {
            ...
            const release = (await get(commitOrId, releaseOpts));
