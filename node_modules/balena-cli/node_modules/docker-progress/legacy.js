...
(function() {
  var ProgressReporter, Promise, Registry, RegistryV1, RegistryV2, _, calculatePercentage, getLongId, getRegistryAndName, request,
  ...
  exports.getRegistryAndName = getRegistryAndName = function(docker, image) {
    return docker.getRegistryAndName(image).then(function(arg) {
  ...
  exports.getLayerDownloadSizes = function(docker, image) {
    return getRegistryAndName(docker, image).then(function(arg) {
      var imageName, registry, tagName;
      registry = arg.registry, imageName = arg.imageName, tagName = arg.tagName;
  ...
  exports.getImageLayerSizes = function(docker, image) {
  ...
  Registry = (function() {
    function Registry(registry, version) {
    ...
    Registry.prototype.head = function(path) {
  ...
  exports.RegistryV1 = RegistryV1 = (function(superClass) {
    ...
    function RegistryV1(registry) {
      RegistryV1.__super__.constructor.call(this, registry, 1);
    }
  ...
  exports.RegistryV2 = RegistryV2 = (function(superClass) {
    extend(RegistryV2, superClass);

    function RegistryV2(registry) {
      RegistryV2.__super__.constructor.call(this, registry, 2);
    }
    ...
    RegistryV2.prototype.getLayerDownloadSizes = function(imageName, tagName) {
      return this.getImageLayers(imageName, tagName).then((function(_this) {
        return function(blobIds) {
          var layerSizes;
          layerSizes = {};
          return Promise.map(blobIds, function(blobId) {
  ...
  exports.ProgressReporter = ProgressReporter = (function() {
    function ProgressReporter(renderProgress, docker1) {
      this.renderProgress = renderProgress;
      this.docker = docker1;
    }

    ProgressReporter.prototype.getRegistryAndName = function(image) {
    ...
    ProgressReporter.prototype.getLayerDownloadSizes = function(image) {
    ...
    ProgressReporter.prototype.pullProgress = function(image, onProgress) {
      ...
      return this.getLayerDownloadSizes(image).spread(function(registryVersion, layerSizes, remoteLayerIds) {
        var currentDownloadedSize, currentExtractedSize, layerIds, progressMultiplier, totalDownloadedSize, totalExtractedSize, totalSize;
        layerIds = {};
        ...
        return function(evt) {
          var downloadedPercentage, downloadedSize, err, extractedPercentage, extractedSize, percentage, ref, remoteId, shortId, status;
          try {
            status = evt.status;
            shortId = evt.id;
            ...
            if (status === 'Downloading') {
              ...
              currentDownloadedSize[shortId] = 0;
              currentExtractedSize[shortId] = 0;
            } else if (status.startsWith('Status: Image is up to date for ') || status.startsWith('Status: Downloaded newer image for ')) {
            ...
            percentage = Math.floor((downloadedPercentage + extractedPercentage) / progressMultiplier);
    ...
    ProgressReporter.prototype.getImageLayerSizes = function(image) {
      var lastLayer, layers;
    ...
    ProgressReporter.prototype.pushProgress = function(image, onProgress) {
      ...
      return this.getImageLayerSizes(image).then(function(layerSizes) {
        var completeStatuses, completedSize, layerIds, layerPushedSize, totalSize;
        layerIds = _.keys(layerSizes);
        layerPushedSize = {};
        completedSize = 0;
        totalSize = _.sum(_.values(layerSizes));
        completeStatuses = ['Image already exists', 'Layer already exists', 'Image successfully pushed'];
        return function(evt) {
          ...
          try {
            ...
            } else if (pushMatch || _.includes(completeStatuses, status)) {
            ...
            if (status.startsWith('latest: digest: ') || status.startsWith('Pushing tag for rev ')) {
