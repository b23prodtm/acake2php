 ...
 * 	console.log(config)
 */
async function read(image, type) {
    const configuration = await utils.getConfigPartitionInformationByType(type);
    const file = await imagefs.interact(image, configuration.partition, async (fs) => {
        return await util_1.promisify(fs.readFile)(configuration.path, {
            encoding: 'utf8',
        });
    });
 ...
 * 	console.log('Done!')
 */
async function write(image, type, config) {
    config = JSON.stringify(config);
    const configuration = await utils.getConfigPartitionInformationByType(type);
    await imagefs.interact(image, configuration.partition, async (fs) => {
        return await util_1.promisify(fs.writeFile)(configuration.path, config);
    });
}
exports.write = write;
