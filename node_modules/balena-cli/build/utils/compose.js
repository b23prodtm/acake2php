...
exports.BuildProgressInline = exports.BuildProgressUI = exports.pushAndUpdateServiceImages = exports.authorizePush = exports.getPreviousRepos = exports.tagServiceImages = exports.createRelease = exports.originalTarDirectory = exports.createProject = exports.generateOpts = void 0;
...
function generateOpts(options) {
    ...
    if (options.gitignore && options['multi-dockerignore']) {
        throw new errors_1.ExpectedError('The --gitignore and --multi-dockerignore options cannot be used together');
    }
...
function createProject(composePath, composeStr, projectName = null) {
    ...
    projectName !== null && projectName !== void 0 ? projectName : (projectName = path.basename(composePath));
...
async function originalTarDirectory(dir, param) {
    let { preFinalizeCallback = null, convertEol = false, nogitignore = false, } = param;
    if (convertEol == null) {
        convertEol = false;
    }
    ...
    const getFiles = () => Bluebird.resolve(streamToPromise(klaw(dir)))
    ...
    return getFiles()
        ...
        .tap(() => {
        if (!nogitignore) {
            printGitignoreWarn((ignoreFiles[IgnoreFileType.DockerIgnore] || [])[0] || '', ignoreFiles[IgnoreFileType.GitIgnore] || []);
        }
    })
        .filter(ignore.filter)
        .map(function (file) {
        const relPath = path.relative(path.resolve(dir), file);
        return Promise.all([relPath, fs.stat(file), readFile(file)]).then(([filename, stats, data]) => pack.entry({
        ...
        .then(() => preFinalizeCallback === null || preFinalizeCallback === void 0 ? void 0 : preFinalizeCallback(pack))
        .then(function () {
        pack.finalize();
        return pack;
    });
}
exports.originalTarDirectory = originalTarDirectory;
exports.createRelease = async function (apiEndpoint, auth, userId, appId, composition) {
    ...
    return {
        ...
        serviceImages: _.mapValues(serviceImages, (serviceImage) => _.omit(serviceImage, [
            'created_at',
...
exports.tagServiceImages = (docker, images, serviceImages) => Promise.all(images.map(function (d) {
...
exports.getPreviousRepos = (sdk, docker, logger, appID) => sdk.pine
    .get({
    resource: 'release',
    options: {
        $filter: {
...
exports.authorizePush = function (sdk, tokenAuthEndpoint, registry, images, previousRepos) {
...
exports.pushAndUpdateServiceImages = function (docker, token, images, afterEach) {
    ...
    const renderer = pushProgressRenderer(tty, lazy_1.getChalk().blue('[Push]') + '    ');
    const reporters = progress.aggregateProgress(images.length, renderer);
    return Bluebird.using(tty.cursorHidden(), () => Promise.all(images.map(({ serviceImage, localImage, props, logs }, index) => Promise.all([
        localImage.inspect().then((img) => img.Size),
        retry({
            func: () => progress.push(localImage.name, reporters[index], opts),
            maxAttempts: 3,
            label: localImage.name,
            initialDelayMs: 2000,
            backoffScaler: 1.4,
        }).finally(renderer.end),
    ])
        .then(function ([size, digest]) {
        ...
        .finally(() => afterEach === null || afterEach === void 0 ? void 0 : afterEach(serviceImage, props)))));
};
...
class BuildProgressUI {
    constructor(tty, descriptors) {
        ...
        const prefix = lazy_1.getChalk().blue('[Build]') + '   ';
        const offset = 10;
        this._prefixWidth =
    ...
    _renderStatus(end) {
        end !== null && end !== void 0 ? end : (end = false);
        ...
        if (end && this._cancelled) {
            ...
            const durationStr = this._startTime == null
                ? 'unknown time'
                : moment
                    .duration(Math.floor((Date.now() - this._startTime) / 1000), 'seconds')
                    .format();
...
class BuildProgressInline {
    ...
    _renderEvent(service, event) {
        ...
        const prefix = _.padEnd(lazy_1.getChalk().bold(service), this._prefixWidth);
