...
if (filter_input(INPUT_GET, 'ajouter') === "publie" || filter_input(INPUT_GET, 'modifier') === "publie") {
        ...
        $date = getdate(time());
        $info = new Info($sql, FALSE, filter_input(INPUT_POST, 'i_titre'), filter_input(INPUT_POST, 'i_auteur'), filter_input(INPUT_POST, 'i_categorie'), $date['year'] . "-" . $date['mon'] . "-" . $date['mday']);
        ...
        if ($info->publier($sql, $update)) {
                $pAdmin->ajouterContenu("<b><center><br>Info - " . filter_input(INPUT_POST, 'i_titre') . " - publiée sur le site!</center></b>");
                if (filter_input(INPUT_GET, 'modifier')) {
